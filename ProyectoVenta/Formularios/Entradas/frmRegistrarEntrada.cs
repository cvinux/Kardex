...
namespace ProyectoVenta.Formularios.Entradas
{
    public partial class frmRegistrarEntrada : Form
    {
        ...
        private void txtlote_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            {
                if (txtlote.Text.Trim().Length == 0 && e.KeyChar.ToString() == ".")
                {
                    e.Handled = true;
                }
                else
        ...
        private void btnagregar_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Debe ingresar el lote de producto", "Mensaje", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }

            if (txtfechaven.Text.Trim() == "")
            {
                MessageBox.Show("Debe ingresar la fecha de vencimiento del lote", "Mensaje", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }
            ...
            DateTime dt1 = Convert.ToDateTime(txtfechaven.Value.ToString("dd/MM/yyyy"));
            subtotal = Convert.ToDecimal(txtcantidad.Value.ToString()) * Convert.ToDecimal(txtpreciocompra.Text.ToString());
            descuento = (Convert.ToDecimal(txtdescuento.Text.ToString()) /100)* subtotal;
        ...
        private bool producto_agregado() {
            ...
            if (dgvdata.Rows.Count > 0) {
                foreach (DataGridViewRow fila in dgvdata.Rows)
                {
                    ...
                    {
                        if (fila.Cells["Lote"].Value.ToString() == txtlote.Text)
                        {
                            respuesta = true;
                            break;
                        }
        ...
        private void btnguardarentrada_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataGridViewRow row in dgvdata.Rows)
            {
                olista.Add(new DetalleEntrada() {
                    ...
                    TotalDescuento = row.Cells["TotalDescuento"].Value.ToString(),
            ...
            Entrada oEntrada = new Entrada() {
                ...
                Lote = txtlote.Text,
                FechaVencimiento = txtfechaven.Value.ToString("dd/MM/yyyy"),
                Descuento = txtdescuento.Text,
                TotalDescuento =lbltotal.Text,
